 ...
 */
    ...
    $suxnix_video_url = function_exists( 'get_field' ) ? get_field( 'fromate_style' ) : NULL;
...
<?php if ( is_single() ) : ?>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'blog--post--item mb-50 format-video' );?>>

        <?php if ( has_post_thumbnail() ): ?>
        <div class="blog--post--thumb position-relative">
            <?php the_post_thumbnail( 'full', ['class' => 'img-responsive'] );?>
            <?php if(!empty($suxnix_video_url)) : ?>
                <a href="<?php print esc_url( $suxnix_video_url );?>" class="video--icon popup-video"><i class="fas fa-play"></i></a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
...
<?php else: ?>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'blog--post--item mb-40 format-video' );?> >
