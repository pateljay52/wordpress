 ...
 */
    ...
    $suxnix_audio_url = function_exists( 'get_field' ) ? get_field( 'fromate_style' ) : NULL;
...
<?php if ( is_single() ) : ?>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'blog--post--item mb-50 format-audio' );?>>

        <?php if ( !empty( $suxnix_audio_url ) ): ?>
        <div class="blog--post--thumb postbox__audio ratio ratio-16x9">
            <?php echo wp_oembed_get( $suxnix_audio_url ); ?>
        </div>
        <?php endif;?>
...
<?php else: ?>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'blog--post--item mb-40 format-audio' );?> >
